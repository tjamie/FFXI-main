...
function precast(spell)
    ...
    -- Job Abilities
    -- We use a catch all here,
    -- if the set exists for an ability, use it
    -- This way we don't need to write a load of different code for different abilities, just make a set
    if sets.precast[spell.name] then
        equip(sets.precast[spell.name])        
    end

    -- grimoire precast here, adjust main sch.lua accordingly
...
function midcast(spell)
    ...
    elseif spell.skill == 'Enhancing Magic' then
        ...
        --         equip(sets.midcast.refresh_self)
        --     else
        ...
        -- Ignore phalanx for now and add a thing to detect accession later on, ie, when we know everything else is working
    ...
    -- Weapon skills
    -- sets.me["Insert Weaponskill"] are basically how we define any non-magic spells sets, aka, WS, JA, Idles, etc.
...
function self_command(command)
    ...
    if #commandArgs:split(' ') >= 2 then
        ...
        elseif commandArgs[1]:lower() == 'sc' then
            ...
            if arg == 'castsc' then
                -- TODO add a toggle for party messages
                -- TODO update with Tier II helices unless they shouldn't be bursted for some reason i'm not aware of
...
if player and player.index and windower.ffxi.get_mob_by_index(player.index) then
    ...
    --
    -- Commenting out RDM movement logic for carmine cuisses in case I decide to use it later for some reason
    --
    -- mov = {counter=0}
    -- if player and player.index and windower.ffxi.get_mob_by_index(player.index) then
