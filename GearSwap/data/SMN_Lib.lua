...
function precast(spell)
    if (pet.isvalid and pet_midaction() and not spell.type=="SummonerPact") or spell.type=="Item" then
        -- Do not swap if pet is mid-action. + type=SummonerPact check because sometimes when the avatar
...
function status_change(new,old)
    ...
    elseif not midaction() and not pet_midaction() then
        idle()
    end
    -- else
    --     idle()
...
function self_command(command)
    ...
    elseif commandArgs:lower() == 'accmode' then
